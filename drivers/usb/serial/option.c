			...
			const struct usb_device_id *id)
{
	...
	//Compal Start Terry 2011-11-30
	/*Add auto suspend by lenovo patch*/
	//if (serial->dev->descriptor.idVendor == HUAWEI_VENDOR_ID){
	/*Begin: Added by Dangxf*/
	if (serial->dev->descriptor.idVendor == HUAWEI_VENDOR_ID) {
		g_hw_udev = interface_to_usbdev(serial->interface);
		printk("%s:usb_disable_autosuspend\n",__func__);
		if(g_hw_udev)
			//usb_enable_autosuspend(g_hw_udev);
			usb_disable_autosuspend(g_hw_udev);
	}
	//Compal End Terry 2011-11-30
