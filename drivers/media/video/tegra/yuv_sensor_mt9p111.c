...
#include "yuv_mt9p111_setting_tab_k1.h"
...
static u8 focusmode_pos_near = 0x40;
static u8 focusmode_pos_mid = 0x20;
static int lowlight_threshold = 0;
static u16 targetexposure_reg = 0xA409;
static u8 preview_ae_target = 0x46;
/* Compal K1 - Carl end */
...
static int sensor_write_table(struct i2c_client *client, const struct sensor_reg table[])
{
	...
	/* Compal K1 - Carl begin */
		case CHECK_FLASH:
		{
			pr_info("[mt9p111] CHECK_FLASH ++++\n");  //Carl test
			u8 val1, val2;
			err = sensor_read_reg8(info->i2c_client, focusmode_reg, &val1);
			if (err)
			    return err;
			err = sensor_read_reg8(info->i2c_client, targetexposure_reg, &val2);
			if (err)
			    return err;
			preview_ae_target = val2;

			if (info->focus_mode == SENSOR_AF_FULLTRIGER) {
			    if (((info->flash_mode == YUV_5M_FlashControlAuto) && (lowlight_threshold == 1)) || 
				(info->flash_mode == YUV_5M_FlashControlOn))
			    {
				if (val1 >= focusmode_pos_near) {
				    val2 = (preview_ae_target < 0x10) ? preview_ae_target : 0x10;
				    pr_info("[mt9p111] CHECK_FLASH (AF_FULLTRIGER) : set AE target = 0x%x\n", val2);  //Carl debug
				    err = sensor_write_reg8(info->i2c_client, targetexposure_reg, val2);
				} else if (val1 >= focusmode_pos_mid) {
				    val2 = (preview_ae_target < 0x28) ? preview_ae_target : 0x28;
				    pr_info("[mt9p111] CHECK_FLASH (AF_FULLTRIGER) : set AE target = 0x%x\n", val2);  //Carl debug
				    err = sensor_write_reg8(info->i2c_client, targetexposure_reg, val2);
				} else {
				    val2 = (preview_ae_target < 0x40) ? preview_ae_target : 0x40;
				    pr_info("[mt9p111] CHECK_FLASH (AF_FULLTRIGER) : set AE target = 0x%x\n", val2);  //Carl debug
				    err = sensor_write_reg8(info->i2c_client, targetexposure_reg, val2);
				}
				if (err)
				...
				pr_info("[mt9p111] CHECK_FLASH : set AE target = 0x%x\n", preview_ae_target);  //Carl debug
				err = sensor_write_reg8(info->i2c_client, targetexposure_reg, preview_ae_target);
				if (err)
				    return err;
			    }
			    ...
			    {
				val2 = (preview_ae_target < 0x28) ? preview_ae_target : 0x28;
				pr_info("[mt9p111] CHECK_FLASH (AF_INFINITY) : set AE target = 0x%x\n", val2);  //Carl debug
                                err = sensor_write_reg8(info->i2c_client, targetexposure_reg, val2);
                                if (err)
				    return err;
			    } else {
...
static int check_otpm_module(struct i2c_client *client)
{
	...
	pr_info("yuv5 %s: read 0xE026, module id = %x\n", __func__, mid);

       if (mid == 0xA5 || mid == 0x32)
               moduleID = 1;  // Chicony Module
       else if (mid == 0xFF)
               moduleID = 2;  // Lite-on Module
       else if (mid == 0x59 || mid == 0x86)
               moduleID = 3;  // Bison Module
       else
           moduleID = 0; // check module fail
...
static int sensor_set_mode(struct sensor_info *info, struct sensor_5m_mode *mode)
{
	...
	if ((val == SENSOR_5M_640_WIDTH_VAL) && (sensor_table == preview_mode)) {
		pr_info("yuv5 %s: ((val == SENSOR_5M_640_WIDTH_VAL) && (sensor_table == preview_mode))\n", __func__);
		/* Compal K1 - Carl begin */
		// recover AE target to avoid abnormal brightness after flash
		err = sensor_write_reg8(info->i2c_client, targetexposure_reg, preview_ae_target);
		/* Compal K1 - Carl end */
		err = sensor_write_table(info->i2c_client, is_preview);
		if (err)
	...
	if (!(val == ONE_TIME_TABLE_CHECK_DATA)) {
		...
		err = sensor_write_table(info->i2c_client,flash_off);
		if (err)
		    return err;
	    /* Compal K1 - Carl end */
	    ...
	    /* Compal K1 - Carl begin */
	    // K1 has no OTP module, load default calibration settings
		/*
		if (otpm_mode == 0) {
		...
		*/
		...
		/*
			...
			if (info->module_source == 1) {
			    pr_info("yuv5 %s: OTPM module is Chicony\n", __func__);
			    sensor_write_table(info->i2c_client, enable_PGA_OTPM_table);
			} else if (info->module_source == 2) {
			    pr_info("yuv5 %s: OTPM module is Lite-on\n", __func__);
			    sensor_write_table(info->i2c_client, enable_PGA_OTPM_table);
			} else if (info->module_source == 3) {
			    pr_info("yuv5 %s: OTPM module is Bison\n", __func__);
			    sensor_write_table(info->i2c_client, enable_PGA_OTPM_table);
			}
		}
		else
...
static long sensor_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_5M_IOCTL_GET_BRIGHTNESS:
	{
	    u16 val;
	    int num;
	    ...
	    pr_info(" %s: SENSOR_IOCTL_GET_BRIGHTNESS ...+ \n",__func__); 
            ...
            err = sensor_read_reg(info->i2c_client, 0xB80C, &val);
	    pr_info("%s: value %x \n",__func__, val);
	...
	case SENSOR_5M_IOCTL_SET_EXPOSURE:
	{
		...
		/* Compal K1 - Carl begin */
		// keep AE target for AE recover
		err = sensor_read_reg8(info->i2c_client, targetexposure_reg, &exposure);
		preview_ae_target = exposure;
		pr_info("[mt9p111] set preview_ae_target = 0x%x\n", preview_ae_target);  //Carl debug
		/* Compal K1 - Carl end */
		return 0;
	}
//#if 0  // Control by HAL
