...
//Charlie 2012.5.4 merge NV patch to fix regarding to soft lock up which caused by cam i2c
#if 0
...
#else
	//specially for CAM_I2C.
	if ( i2c_dev->cont_id == 2 && i2c_readl(i2c_dev, I2C_INT_MASK) == 0x0 )
	{
                if (!i2c_dev->irq_disabled) {
			disable_irq_nosync(i2c_dev->irq);
                        i2c_dev->irq_disabled = 1;
