					...
					struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event))
	{
		printk("Eric ext_mic event on\n");
		snd_soc_write(Mycodec, WM8903_ANALOGUE_LEFT_INPUT_0,0x1B);
		snd_soc_write(Mycodec, WM8903_ANALOGUE_RIGHT_INPUT_0,0x1B);
        	snd_soc_write(Mycodec, WM8903_ADC_DIGITAL_VOLUME_LEFT, 0x85);
        	snd_soc_write(Mycodec, WM8903_ADC_DIGITAL_VOLUME_RIGHT, 0x85);
        	snd_soc_write(Mycodec,WM8903_ANALOGUE_RIGHT_INPUT_1,0x10);
        	snd_soc_write(Mycodec,WM8903_ANALOGUE_LEFT_INPUT_1,0x10);
	}
	else
	{
		printk("Eric ext_mic event off\n");
	}
